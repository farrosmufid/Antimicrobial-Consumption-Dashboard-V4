...
| Chart | Description | Visualisation |
| :-: | - | - |
| **Filters** | The resistance page has 8 main filters that apply to all 4 visualisations. They include a date range, a number of tests requirement, the type of orderable, the medical service, the antibiotic type, the organism, the nurse location and finally the medical officer.

The date range filter allows the user to decide the period of interest to investigate susceptibility data. The format of the date is YYYY-MM-DD. You can either choose to type the date explicitly, or click on the date field and a calendar selector dropdown will appear and you can select the start and end date from there. Clicking the “?” in the bottom right corner of the calendar dropdown will bring up a list of keyboard shortcuts when using the calendar selector

As outlined by the Australian Commision on Safety and Quality in Health Care in their “Specification for a Hospital Cumulative Antibiogram” report (https://www.safetyandquality.gov.au/publications-and-resources/resource-library/specification-hospital-cumulative-antibiogram-2019):
“Fewer than 30 isolates of any grouping of bacteria do not provide statistically significant information. When data from fewer than 30 isolates are presented, a comment should be included to reflect this and, to provide context.” Therefore, as default the selection for this filter is “30+ Tests Required”. This will mean in all the charts except for the line chart, only results of susceptibility will be shown in antibiotics/organisms that have at least 30 isolates. However if the user is interested in seeing all data for particular antibiotics for example, they have the ability to select “All” which will show all rates no matter the number of isolates, however it is important to be aware that these results may not be statistically significant

The other 6 filters interact the same. As default when nothing has been specifically chosen then all samples are selected. A user can select more than one option at a time.

It is important to know that all the filters interact with each other dynamically. Therefore for example if a user selects a medical service of Dermatology and then a specific antibiotic Cefatoxin of interest, when they go to select the nurse location only two available values are able to be selected, because they are the only relevant values for the existing selection.
